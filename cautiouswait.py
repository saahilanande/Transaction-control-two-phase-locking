    ...
    if lockcheck.size > 0:
        ...
        for items in lockcheck.iterrows():
            if items[1][0] == data and items[1][1] == 'wl':
                ...
                break
        if flag:
            #if the lock is state is write lock perform cautious wait
            cautious(id,lockcheck,data,'r')
...
#if the data is write lock or readlock by another transation perform cautious wait
    else:

        cautious(id,lockcheck,data,'w')


def cautious(id,lockcheck,data,operation):
    global transtable
    global  locktable
    global  blocktable
    ...
    print("performing causiois waiting")

    for x in lockcheck['lockingtransacion'].to_list():
        if x != id:
            trans = transtable.loc[transtable['transid'] == x, ['transid','tstamp','status','itemslocked']]
            status = trans['transid'].to_list()[0]
            list.append(x)
            if status == 'blocked':
                 flag = True
    ...
    if flag:
        ...
        for y in list:
            blocktable = blocktable.append(pd.Series([id, data, operation, y], index=blocktable.columns),ignore_index=True)
...
def abort(id):
    ...
    for x in list.iterrows():
        ...
        if getcount.size > 0:
            ...
            newtranshold = getcount['lockingtransacion'].to_list()[0]

            list = blocktable.loc[(blocktable['lockingtransacion'] == id) & (blocktable['transid'] != tranid), ['transid','item','operation','lockingtransacion']]

    changedList = blocktable.loc[(blocktable['lockingtransacion'] == id) & (blocktable['transid'] == blockedid), ['transid', 'item','operation', 'lockingtransacion']]

    for lines in changedList.iterrows():

        blocktable.loc[blocktable['transid']==blockedid, ['lockingtransacion']] = newtranshold
...
def commit(id):
    ...
    for x in list.iterrows():
        ...
        if getcount.size > 0:
            ...
            newblockby = getcount['lockingtransacion'].to_list()[0]
    ...
    for lines in changedList.iterrows():
        ...
        blocktable.loc[blocktable['transid'] == blockedid, ['lockingtransacion']] = newblockby
    ...
    for row in list.iterrows():
        ...
        if getcountblock.size > 1:
            ...
            #execute the operation blocked by current transcation
            blocktable = blocktable.loc[(blocktable['item'] != dataitem) | (blocktable['lockingtransacion'] != id)]
            outputfile.write("Operations " + row[1][2] + " in queue are executed")
            outputfile.write('\n')
            transtable.loc[transtable['transid'] == tranid, ['status']] = 'active'
...
outputfile = open("output4cautiouswait.txt", 'w')
